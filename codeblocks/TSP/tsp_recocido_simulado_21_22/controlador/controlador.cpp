...
void Controlador::registra_observadores()
{
    ...
    algoritmo_rs.registra_observador(rs::Evento_modelo::nueva_iteracion,[this]()
    {
        //ejecutar un metodo que llama a settext
        actualiza_contador();
    });
...
//Pongo un configuracion de muestra para el calculo de la temperatura
    parametros.num_descensos = ciudades.size();
    parametros.num_permutaciones = ciudades.size();
    parametros.num_exitos_maximo = 10*ciudades.size();
    parametros.temperatura = std::numeric_limits<double>::max();
    parametros.factor_descenso = 0.999;
    bool temperatura_optima=false;
    unsigned num_exitos_totales;
    double banda_superior = std::numeric_limits<double>::max();
    double banda_inferior =0;
    double factor=0;
    unsigned iterador=0;

    while(temperatura_optima==false)
    {
        num_exitos_totales = algoritmo_rs.Calcula_temperatura(ruta::Ruta(ciudades), parametros);

        std::cout<<"Temperatura: "<<parametros.temperatura<<std::endl;
        std::cout<<"num_exitos_maximo: "<<parametros.num_exitos_maximo<<std::endl;
        std::cout<<"num_exitos_totales: "<<num_exitos_totales<<std::endl;
        std::cout<<"banda superior: "<<banda_superior<<std::endl;
        std::cout<<"banda_inferior: "<<banda_inferior<<std::endl;

        //Dos formas: 1º forma:
        if(num_exitos_totales == 0)
            break;
        if(num_exitos_totales>(0.61*parametros.num_exitos_maximo))
        {
            banda_superior=parametros.temperatura;
            factor=(banda_superior-banda_inferior)/2.0;
            //Tiene m�s aceptacion de la que deberia-> reduzco la temperatura
            parametros.temperatura=parametros.temperatura-factor;
            std::cout << "Bajo" << std::endl;
        }
        else if(num_exitos_totales<(0.59*parametros.num_exitos_maximo))
        {
            banda_inferior=parametros.temperatura;
            factor=(banda_superior-banda_inferior)/2.0;
            //Tiene menos aceptacion de la que deberia-> aumento la temperatura
            parametros.temperatura=parametros.temperatura+factor;
            std::cout << "Subo" << std::endl;
        }
        else
        {
            std::cout<<"Iteraciones: "<<iterador<<std::endl;
            temperatura_optima=true;
        }
        iterador++;
    }

    //Confiuro bien los parametros para el analisis completo
    std::cout<<"Temperatura final: "<<parametros.temperatura<<std::endl;
...
void Controlador::actualiza_contador()
{
    vista.set_texto_iteracion(std::to_string(algoritmo_rs.get_iteracion_actual()));
}
