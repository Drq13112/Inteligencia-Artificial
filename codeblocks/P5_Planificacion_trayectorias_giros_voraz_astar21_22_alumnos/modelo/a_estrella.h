...
namespace problema_busqueda
{
    ...
    Solucion<Problema> a_estrella(Problema& estado_inicial, Problema& estado_objetivo)
    {
        ...
        struct Nodo:Nodo_busqueda  //Herencia
        {
            ...
            {
                return coste_heuristica+coste_ruta < nodo.coste_heuristica+nodo.coste_ruta;
            }
        ...
        while (exito == false && !frontera.empty())
        {
            ...
            if (exito == false)// Expandimos al siguiente nodo
            {
                ...
                for (auto& [op, estado_hijo] : visitados.back().estado.sucesores())
                {
                    ...
                    //std::cout << estado_hijo << " " << coste_ruta_actual << std::endl;
